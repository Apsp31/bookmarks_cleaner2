...
**Confidence:** HIGH (core stack), MEDIUM (bookmark parsing, classification API)
...
```bash
# Core server + parsing
npm install express cheerio fastest-levenshtein p-limit

# No frontend npm install needed — Alpine.js loaded via CDN in HTML
```

```json
// package.json — required for p-limit ESM
{
  "type": "module",
  "engines": { "node": ">=20" }
}
...
**If concurrency tuning is needed:**
...
fastest-levenshtein is a CJS package. In a project with `"type": "module"`, you can either:
1. Use a dynamic `import()` (CJS interop in Node ESM works for most packages)
2. Or switch to `leven` (sindresorhus, ESM-native, 1.2KB) if interop is annoying in practice
...
---
*Stack research for: Local bookmark cleaner — Node.js utility with browser UI*
*Researched: 2026-03-23*
